...
/*
  provisioner "file" {
    source      = "${path.module}/inputfiles/configservice"
    destination = "/tmp/myapp.conf"
    connection {
      user = "centos"
      type = "ssh"
      private_key = "${file("~/ssh/authorized_keys/vault_test.pem")}"
      timeout = "30s"
      agent = false
    }
  }
*/
  template = "${file("${path.module}/scripts/install_centos.sh.tpl")}"

  vars {
      download_url  = "${var.download-url}"
#      config        = "${var.config}"
      extra-install = "${var.extra-install}"
      vaultversion  = "${var.vaultversion}"
      vaultzipfile  = "${var.vaultzipfile}"
  #    vaultservice  = "${var.vaultservice}"
    }
}
